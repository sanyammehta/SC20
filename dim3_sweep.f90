...
MODULE dim3_sweep_module
  ...
  USE global_module, ONLY: i_knd, l_knd, r_knd, zero, two, one, half, ounit
  ...
  USE control_module, ONLY: fixup, tolr, last_oct, update_ptr, cor_swp
...
!    PRINT *, klo, khi, jlo, jhi, iclo, ichi
    
!    111 FORMAT(I3, ',',I3, ',',I3, ',',I3, ',',I3, ',',I3, '\')
...
!DIR$ PREFETCH (level(2)) psik(1,ic+ist,j)
!DIR$ PREFETCH (level(2)) psik(9,ic+ist,j)
!DIR$ PREFETCH (level(2)) psik(17,ic+ist,j)
...
!DIR$ PREFETCH (level(2)) psik(25,ic+ist,j)
!DIR$ PREFETCH (level(2)) psik(33,ic+ist,j)
!DIR$ PREFETCH (level(2)) psik(41,ic+ist,j)
...
!DIR$ PREFETCH (level(2)) psij(1,ic+ist,k)
!DIR$ PREFETCH (level(2)) psij(9,ic+ist,k)
!DIR$ PREFETCH (level(2)) psij(17,ic+ist,k)
...
!DIR$ PREFETCH (level(2)) psij(25,ic+ist,k)
!DIR$ PREFETCH (level(2)) psij(33,ic+ist,k)
!DIR$ PREFETCH (level(2)) psij(41,ic+ist,k)
...
!_______________________________________________________________________
    ...
    IF ( cor_swp == 0 ) CALL sweep_send_bdry ( g, jd, kd, iop, t, reqs,&
      szreq, nc, nang, ichunk, ny, nz, jb_out, kb_out )
!_______________________________________________________________________
!_______________________________________________________________________
